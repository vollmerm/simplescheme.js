  ...
  var parse = function parse(str)
    {
      ...
      for (var i = 0; i < tokens.length; i++)
      {
        ...
        try
        {
          ...
          {
            if (typeof(returned_value) == "object" && returned_value.length > 1)
            {
              for (var i = 0; i < returned_value.length; i++)
              {
                if (is_typed_object(returned_value[i]))
                { 
                  returned_value[i] = returned_value[i].data;
                }
              }
            }

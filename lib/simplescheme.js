...
(function ()
{
  ...
  function recursive_descent(str)
  {
    ...
    function num_or_symbol()
    {
      // special case: decide what to do when a symbol
      // starts with a negative sign
      var buf = "", number = false;
      buf += '-';
      match('-');
      if (char_is_number()) { number = true; }
      while (char_is_letter() || char_is_number() ||
             char_is_symbol())
      {
        if (!char_is_number() && str.charAt(ptr) != ".") { number = false; }
        buf += str.charAt(ptr);
        match(str.charAt(ptr));
      }
      if (number) { return new_num(Jsnum.fromString(buf)); }
      else { return buf; }
    }
    function whitespace()
    {

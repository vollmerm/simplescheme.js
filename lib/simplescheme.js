  ...
  function run_sequence(exp_list, env, caller, tail)
  {
                    ...
                    (typeof (exp_list[0]) == "object"))
    {
      ...
      while (exp_stack.length > 0)
      {
        ...
        // check for functions with no arguments (eval_l misses those)
        if (typeof(ret) == "function")
        {
          ret = ret.call(null,null);
        }
  ...
  var parse = function parse(str)
    {
      ...
      for (var i = 0; i < tokens.length; i++)
      {
        ...
        try
        {
          ...
          // check for functions with no arguments (eval_l misses those)
          if (typeof(returned_value) == "function")
          {
            returned_value = returned_value.call(null,null);
          }

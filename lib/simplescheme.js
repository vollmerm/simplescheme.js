...
(function ()
{
  ...
  // an optional caller (function name) for detecting tail calls, and a 
  // flag for when eval is evaluating a parameter to a function
  function eval_l(x, env, caller, in_param)
  {
    ...
    else if (x[0] == 'define')
    {
      ...
      else if (typeof (x[1]) == 'object' && x[1].length > 1)
      {
        ...
        return eval_l(['define', x[1][0],new_lambda_exp], env);
        
      }
      else if (x[2][0] == "lambda")
      {
    ...
    else
    {
      ...
      if (typeof (x) == "object" && x.length > 0)
      {
        ...
        if (caller && x[0] == caller)
        {
          ...
          {
            evaluated_elements[j] = eval_l(x[j], env, null, true);
          }
  ...
  function build_function(lambda_exp, env, name)
  {
    ...
    var exp = lambda_exp.slice(1);
    var local_context_def;
    var local_context;
    local_context_def = new Context();
    local_context_def.parent_context = env;
    var new_func = function ()
    {
      local_context = new Context();
      local_context.parent_context = local_context_def;
  ...
  function parse(str, debug)
  {
    ...
    for (var i = 0; i < tokens.length; i++)
    {
      ...
      function parse_process()
      {
        ...
        {
          output_string += print_list(returned_value,false).trim();
        }
  ...
  function print_list(list, formatted)
  {
    ...
    if (typeof(list) == "object" && list.length != null)
    {
      for (var i = 0; i < list.length; i++)
      {
        ...
        else if (typeof(list[i]) == "object" && is_typed_object(list[i]))
        {
          str += list[i].data + " ";
        }
        else if (typeof(list[i]) == "object" && list.length != null)
        {
          str += print_list(list[i],false);
        }
        else
        {
          str += '~' + list[i] + " ";
    ...
    str = str.slice(0,str.length-1); // remove trailing space
    if (formatted)
    {
      return str;
    }
